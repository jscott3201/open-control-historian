...
The public parsed view remains non-authorizing. It exposes bounded metadata and
index/frame inspection only; exact frame decode still produces
`DecodedAdmissionV1`, not `CanonicalAdmission`. Closed errors retain no paths,
observations, or unbounded strings.
...
crash convergence, runtime build, query/cursor semantics, multiple-generation
...
The exact sealed raw Journal V1 remains the sole durable authority.
