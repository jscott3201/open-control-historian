...
const SUCCESS_CHILD: &str = "v1-success-smoke";
const FULL_CHILD: &str = "v1-pressure-full-smoke";
const QUOTA_CHILD: &str = "v1-pressure-quota-smoke";
...
impl V1SmokeChild {
    fn acquire(root: &EvidenceRoot, name: &str) -> Result<Self> {
...
pub(super) fn run_success(root: &EvidenceRoot) -> Result<()> {
    run_child(root, SUCCESS_CHILD, |store| {
        let witness = success(store, false)?;
        if witness.handled == "HANDLED" && witness.durable == "DURABLE" {
            Ok(())
        } else {
...
pub(super) fn run_pressure(
    root: &EvidenceRoot,
    kind: InjectedErrorKind,
    partial: bool,
) -> Result<()> {
    let name = match (kind, partial) {
        (InjectedErrorKind::StorageFull, false) => FULL_CHILD,
        (InjectedErrorKind::QuotaExceeded, true) => QUOTA_CHILD,
        _ => return Err(EvidenceError::InvalidHarness),
    };
    run_child(root, name, |store| {
        pressure(store, kind, partial).map(|_| ())
    })
}
...
fn run_child<T>(
    ...
    let mut child = V1SmokeChild::acquire(root, name)?;
    let operation_result = operation(&child.path);
...
fn pressure(store: &Path, kind: InjectedErrorKind, partial: bool) -> Result<RuntimeWitness> {
...
mod tests {
    ...
    #[test]
    fn owner_specific_v1_child_cleanup_preserves_errors_and_allows_retry() {
        let parent = Temp::new("owner-cleanup");
        let root =
            EvidenceRoot::prepare(&parent.0.join("evidence")).expect("prepare evidence root");
        root.foundation_layout().expect("prepare foundation layout");

        assert!(matches!(
            run_child(&root, SUCCESS_CHILD, |_| {
                Err::<(), _>(EvidenceError::InvalidHarness)
            }),
            Err(EvidenceError::InvalidHarness)
        ));
        ...
        assert!(matches!(
            run_child(&root, SUCCESS_CHILD, |store| {
                fs::remove_dir(store).map_err(|_| EvidenceError::Io)?;
